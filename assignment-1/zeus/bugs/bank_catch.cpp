...
bool bank::_addStatic(LPSTR url)
{
  ...
  //Îòêðûâàåì êîíôèãóðàöèþ.
  BinStorage::STORAGE *config = LocalConfig::beginReadWrite();
  if(config == NULL)return false;
    
  //Ïîëó÷åì ñïèñîê ñóùåòâóþùèõ ýëåìåíòîâ.
  bool ok = true;
  DWORD itemListSize;
  LPSTR itemList = (LPSTR)BinStorage::_getItemDataEx(config, LocalConfig::ITEM_BANK_LIST, BinStorage::ITEMF_IS_SETTING, &itemListSize);

  //Åñëè ýëåìåíò ñóùåñòâóåò, èùèì êëîí.
  if(isValidList(itemList, itemListSize))
  {
    ...
    do
    {
      ...
      //Ïðîâåðÿåì äîñòóïíî ëè ìåñòà áîëüøå ÷åì, ðàçìåð èñêîìîãî ýëåìåíòà.
...
bool bank::_removeStatic(LPSTR url)
{
  ...
  //Ïðîâåðÿåì ïðàâèëüíîñòü ñïèñêà.
  bool changed = false;
  if(isValidList(itemList, itemListSize))
  {
    //Ñîçäàåì êîïèþ ñïèñêà äëÿ êîïèðîâàíèÿ â íåãî, ýëåìåíòîâ íå ïîïàâøèõ ïîä ìàñêó.
    ...
    if(newItemList != NULL)  
    {
      ...
      do
      {
        ...
        else
        {
          ...
          *offset = 0; //Çàâåðøàþùèé áàéò ìóëüòè-ñòðîêè.
        }
      }
      while((curItem = Str::_multiStringGetIndexA(curItem, 1)) != NULL);
      ...
      if(changed == true)
      {
        if(offset == newItemList)offset--; //Ïî÷òè àíàëîã BinStorage::ITEMF_COMBINE_DELETE.
